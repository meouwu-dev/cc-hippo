...
export function useCanvasNodes() {
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState(saved?.nodes || createDefaultNodes());
  ...
  const saveCanvas = useCallback((currentNodes) => {
    ...
    const serializable = currentNodes.map(n => ({
      ...
      data: {
        ...
        ...(n.type === 'htmlPreview' ? { url: n.data.url, filename: n.data.filename } : {}),
      },
  ...
  const addNode = useCallback((type, data = {}) => {
    ...
    const nodeKey = type;
    const reactFlowType = type.startsWith('htmlPreview') ? 'htmlPreview' : type;

    // Check if node with this key already exists
    const existing = nodesRef.current.find(n => n._key === nodeKey || n.type === nodeKey);
    if (existing) {
      setNodes(nds => {
        const updated = nds.map(n => (n._key === nodeKey || n.type === nodeKey)
          ? { ...n, data: { ...n.data, ...data } }
          : n
        );
        saveCanvas(updated);
  ...
  const updateNodeData = useCallback((type, data) => {
    setNodes(nds => {
      const updated = nds.map(n => n.type === type ? { ...n, data: { ...n.data, ...data } } : n);
      return updated;
    });
  }, [setNodes]);
