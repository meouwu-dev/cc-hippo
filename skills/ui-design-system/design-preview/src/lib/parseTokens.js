...
export function parseDesignTokens(markdown) {
  ...
  if (rootMatch) {
    ...
    while ((match = propRegex.exec(declarations))) {
      ...
      if (name.match(/color|bg|text|primary|secondary|accent|surface|border|success|warning|error|neutral/i)) {
        tokens.colors[name] = val;
      } else if (name.match(/font|text|heading|body|size|weight|line-height|letter-spacing|family/i)) {
  ...
  const fontMatch = markdown.match(/\*\*(?:Primary\s+)?Font(?:.*?):\*\*\s*(.+)/i);
  if (fontMatch) {
    tokens.typography['_primary-font'] = fontMatch[1].trim();
  }
  const headingFontMatch = markdown.match(/\*\*Heading\s+Font(?:.*?):\*\*\s*(.+)/i);
  if (headingFontMatch) {
    tokens.typography['_heading-font'] = headingFontMatch[1].trim();
  }

  // Extract aesthetic direction
  const aestheticMatch = markdown.match(/\*\*Aesthetic(?:\s+Direction)?:\*\*\s*(.+)/i);
  if (aestheticMatch) {
